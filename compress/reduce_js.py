...
_MIN_VARIABLE_REPLACEMENT_LENGTH = 3  # var abc

_SINGLE_LINE_COMMENTS = ('/"/g', "/'/g")  # JS chars with an un-finished comment character

__EXCLUDE_PUBLIC_METHOD_NAMES = ['constructor',
                                 'addEventListener',
                                 'display',
                                 'onclick',
                                 'onreadystatechange']
...
"""
     IMPORTANT, THE CODE MUST NOT HAVE COMMENTS OR THE REDUCE WILL NOT PROPERLY WORK
...
def __reduce_function_block(function_words, func_name, reduce_data, class_name=None):
    ...
    for i, char in enumerate(function_words):
        ...
        if len(char.strip()) >= _MIN_VARIABLE_REPLACEMENT_LENGTH:
...
def __join_text_comments(search_words):
    ...
    for single_comment in _SINGLE_LINE_COMMENTS:
...
def reduce_js(text, vars_on_functions=True, vars_on_methods=True, public=False, skip_items=None, verbose=True):
    ...
    search_words = re.split(_RE_SPLIT_JS, text)
    ...
    for func_name in sorted(reduce_data.functions.keys()):
        ...
        if not func_name.startswith("__"):
            ...
            if not public or func_name in skip_items + __EXCLUDE_PUBLIC_METHOD_NAMES:
                continue
    ...
    if public:
        ...
        # the methods with the same name accross clases must be renamed equally. Ex:
        #
        # CadSocket.print() & CadViewer.print() --> CadSocket.mp1() & CadViewer.mp1()
        #
        for class_data in reduce_data.clases.values():
            for method_data in class_data.methods.values():

                method_name = method_data.name

                if not method_name.startswith("__") and \
                        method_name not in __EXCLUDE_PUBLIC_METHOD_NAMES and \
    ...
    if public:
        ...
        for class_name, class_data in reduce_data.clases.items():
